...
from benchmark.legend.data import iter_case_paths
from benchmark.legend.data.scene_adapters import normalize_scene, scene_to_case, scene_to_layout
from benchmark.legend.feedback import build_feedback
from benchmark.legend.metrics.aggregate import aggregate_case_results, write_benchmark_metrics_outputs
from benchmark.legend.models import create_model
from benchmark.legend.input_modes import canonicalize_input_mode
...
from benchmark.legend.workflow import BenchmarkAgent
from benchmark.legend.workflow.artifacts import configured_max_repair_iterations, output_settings, save_viewer_scene
from benchmark.legend.workflow.judge_evidence_selector import judge_generation_overrides
from benchmark.legend.workflow.evaluate import evaluate_scene
from benchmark.legend.workflow.state import BenchmarkState
from benchmark.legend.workflow.vlm_judge import VLM_JUDGE_INPUT_JSON_ONLY, resolve_vlm_judge_input_mode
...
def _validate_scene_or_layout_or_raise(scene: object, scene_schema: dict, layout_schema: dict, scene_path: str | Path) -> str:
    ...
    if not layout_errors:
        return "legend_layout"
    raise ValueError(
        f"Input JSON at {scene_path} must match schemas/scene.schema.json or Support/legacy/legend/schemas/legend_layout.schema.json. "
...
def _legend_layout_schema_path(root: Path) -> Path:
    return root / "legend" / "schemas" / "legend_layout.schema.json"
...
def copy_viewer_assets(out_dir: str | Path, project_root: str | Path) -> None:
    viewer_src = Path(project_root) / "legend" / "web" / "viewer"
