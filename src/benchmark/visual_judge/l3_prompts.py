...
L3_METRIC_PHASE_PROMPTS = {
    ...
    "semantic_placement_consistency": {
        ...
        "residual_global_placement_review": (
            ...
            "major furnishings or companion sets. Sparsity, asymmetry, or an "
            "open-plan layout alone is not a defect. However, a visually and "
            "geometrically supported collective distribution failure—such "
            "as nearly all activity groups collapsing onto the perimeter "
            "while a large usable central region is left without an "
            "interpretable circulation or activity purpose—may be one "
            "scene_zone defect. Do not score aesthetic preference or vague "
            "oddness. Use scene_program.scene_type only as a broad "
            ...
            "complete, synthesize the final scene verdict. An ordinary scored "
            "finding must name exactly one misplaced subject, optional "
            "non-owning context IDs, and map to scene_zone or "
            "contextual_anchor. For one collective scene-zone distribution "
            "failure, choose one major anchor object as the accounting subject, "
            "restricted to collective_scene_zone_contract."
            "accounting_subject_ids, "
            "put all materially involved objects in context_ids, and use defect "
            "scope=collective_scene_zone_distribution; this is one capped "
            "scene-level event, not one defect per object. It must pass a "
            "reconfiguration test and remain independently abnormal after "
            "granting ordinary function, orientation, access, and clearance. "
            "Do not repeat the same typed physical claim merely under new "
            "wording; overlap with a subject from a different typed claim is "
            "not by itself suppression. Do not restate "
