...
the exact same prompt and ordered image bytes.  Disagreement becomes
``unresolved`` rather than a majority-vote score.
"""
...
COUNTER_STRIKE_VISUAL_JUDGE_VERSION = "counter_strike_visual_judge_v2"
COUNTER_STRIKE_SELECTOR_PROMPT_VERSION = "counter_strike_regional_selector_v1"
COUNTER_STRIKE_METRIC_PROMPT_VERSION = "counter_strike_static_design_judge_v2"
...
are four frozen regional candidate views captured from the original Three.js
runtime. Select zero, one, or two view IDs that best resolve the stated missing
visual fact. Return exactly one JSON object:
...
The original-runtime global views are primary visual evidence. Optional
regional views were selected from a frozen bank without seeing or returning a
verdict. The neutral geometry-derived observation aid is not ground truth:
...
number from 0 to 1 otherwise. If evidence is insufficient, verdict must be
ambiguous, missing_evidence must be non-empty, and defects plus the
metric-specific findings array must be empty. Invalid requires at least one
explicit significant metric-scoped defect. Confidence is intentionally not
requested and must not be invented."""
...
METRIC_RUBRICS = {
    "zone_clarity": (
        ...
        "flank regions. A role need not have text signage or a unique texture; "
        "geometry, enclosure, entrances, and route convergence can establish "
        "it. Do not re-score route count, spawn balance, cover diversity, or "
        "visual style. role_findings must list each required role with "
        "status clear, weak, or missing and cite visible/geometric evidence. "
        "Assign clear only when you can cite specific geometry that delimits "
        "the role: enclosure, a threshold or doorway, backing walls, a height "
        "change, or approaches that visibly converge. Assign weak when the "
        "layout is consistent with the role but no such geometry marks it, so "
        "the region has to be inferred from the overall shape of the arena "
        "rather than seen. Assign missing when no region serves the role. A "
    ...
    "landmark_legibility": (
        ...
        "easy to name and reuse as location or engagement callouts. A landmark "
        "must be visible in the supplied original-runtime views, have a concise "
        "appearance-based name, and be distinguishable from repeated generic "
        "boxes/walls. Do not count spawn markers drawn only on the topology "
        "diagram, bots, HUD, or inferred invisible objects. landmarks must list "
        "name, view_ids, visible_cue, and spatial_region."
    ),
    "cover_diversity": (
        "Judge whether the static arena visibly provides at least four "
        "meaningfully different cover configurations. Differences must be "
        "supportable in cover height/profile, effective width/profile, shape, "
        "or spatial arrangement; repeated generic boxes or mesh fragments of "
        "one structure must not be inflated into extra forms. At least two "
        "height profiles, two width profiles, and two arrangement types must "
        "be represented. Density alone is irrelevant. cover_findings must "
        "list each distinct visible form with form_name, view_ids, "
        "visible_cue, spatial_region, height_profile, width_profile, and "
        "arrangement. height_profile must be exactly low, waist, standing, or "
...
class CounterStrikeVisualJudge:
    ...
    def judge_metric(
        ...
        *,
        ...
        global_packet = tuple(evidence.global_views)
        ...
        if any(
            ...
            selector_result = self._select_regional(
                metric,
                candidates=evidence.regional_views,
            ...
            by_id = {item.id: item for item in evidence.regional_views}
            selected = tuple(by_id[view_id] for view_id in selected_ids)
            if selected:
                phase = "global_plus_regional_fallback"
    ...
    def _combine(
        ...
        *,
        ...
        labels = {
            (item["evidence_status"], item["verdict"]) for item in repeats
        }
        ...
        require_agreement = bool(
            self.config.raw["visual_evidence"]["judge"][
                "require_repeat_agreement"
            ]
        )
        if require_agreement and len(labels) != 1:
        ...
        evidence_status, verdict = next(iter(labels))
        if evidence_status == "insufficient" or verdict == "ambiguous":
            status = "unresolved"
            score = None
            verdict = "ambiguous"
        else:
            status = "checked"
            score = float(
                sum(float(item["score"]) for item in repeats) / len(repeats)
            )
        return CounterStrikeVisualMetricResult(
            metric=metric,
            status=status,
...
def _judge_user_prompt(
    ...
    *,
    ...
    context = {
        ...
        "response_schema": _metric_response_template(metric),
...
def _metric_response_template(metric: str) -> dict[str, Any]:
    common: dict[str, Any] = {
    ...
    if metric == "zone_clarity":
        ...
        common["landmarks"] = [
            {
                "name": "appearance-based callout",
                "view_ids": ["supplied_original_runtime_view_id"],
                "visible_cue": "visible cue",
                "spatial_region": "distinct spatial region",
            }
        ]
    elif metric == "cover_diversity":
        common["cover_findings"] = [
            {
                "form_name": "distinct visible cover form",
                "view_ids": ["supplied_original_runtime_view_id"],
                "visible_cue": "visible cue",
                "spatial_region": "spatial region",
                "height_profile": "waist",
    ...
    if metric == "landmark_legibility":
        ...
        if verdict == "valid" and (
            ...
            raise CounterStrikeVisualJudgeError(
                ...
                "distinct named landmarks and spatial regions",
            )
        ...
        if verdict == "valid":
            ...
            if (
                ...
                raise CounterStrikeVisualJudgeError(
                    ...
                    "distinct forms, height/width profiles, and arrangements",
                )
...
def _validate_role_finding(value: Any) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != {
        ...
        raise CounterStrikeVisualJudgeError(
            "metric_response_schema_invalid",
            "each role finding requires role, status, evidence",
        )
