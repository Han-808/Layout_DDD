...
def materialize_multi_room_evaluation_dataset(
    ...
    *,
    ...
    reusable_case_roots: Mapping[str, str | Path] | None = None,
    ...
    _seed_reusable_cases(
        building=building,
        output=output,
        inventory=inventory,
        dataset_id=dataset_id,
        reusable_case_roots=reusable_case_roots,
    )
    rendered = 0
    resumed = 0
    for room in inventory.succeeded:
...
def _seed_reusable_cases(
    *,
    building: Path,
    output: Path,
    inventory: MultiRoomEvaluationInventory,
    dataset_id: str,
    reusable_case_roots: Mapping[str, str | Path] | None,
) -> None:
    if reusable_case_roots is None:
        return
    declared = {
        str(case_id): Path(path).expanduser().resolve()
        for case_id, path in reusable_case_roots.items()
    }
    expected = {room.case_id for room in inventory.succeeded}
    unknown = sorted(set(declared) - expected)
    if unknown:
        raise ValueError(f"reusable case roots contain unknown case IDs: {unknown}")
    rooms = {room.case_id: room for room in inventory.succeeded}
    for case_id in sorted(declared):
        destination = building / case_id
        if destination.exists() or destination.is_symlink():
            continue
        source = declared[case_id]
        _reject_reusable_case_overlap(
            source,
            output=output,
            building=building,
            case_id=case_id,
        )
        temporary = building / f".{case_id}.reusing"
        if temporary.exists() or temporary.is_symlink():
            _discard_reusable_case(temporary, parent=building, case_id=case_id)
        try:
            _clone_case_tree(source, temporary)
            _verify_ready_case(
                temporary,
                room=rooms[case_id],
                dataset_id=dataset_id,
...
def _reject_reusable_case_overlap(
    source: Path,
    *,
    output: Path,
    building: Path,
    ...
    if source.is_symlink() or not source.is_dir():
        raise ValueError(f"reusable case root must be a real directory: {case_id}")
    for target in (output.resolve(), building.resolve()):
        if source == target or source in target.parents or target in source.parents:
            raise ValueError("reusable case root must be disjoint from output roots")


def _clone_case_tree(source: Path, destination: Path) -> None:
    if destination.exists() or destination.is_symlink():
...
def _discard_reusable_case(path: Path, *, parent: Path, case_id: str) -> None:
    expected = parent / f".{case_id}.reusing"
    if path != expected or path.parent != parent or path.is_symlink():
        raise ValueError("unsafe reusable case path")
    if not path.is_dir():
        raise ValueError("reusable case path is not a directory")
    shutil.rmtree(path)
