...
def _validate_room_evaluator_inputs(
    ...
    *,
    ...
    declared_source_plan_hash = object_plan.get(
        "source_object_plan_canonical_sha256"
    )
    if declared_source_plan_hash not in _source_object_plan_identity_hashes(
        source_plan
    ):
        raise ValueError("source room object-plan canonical hash mismatch")


def _source_object_plan_identity_hashes(
    source_plan: Mapping[str, Any],
) -> frozenset[str]:
    """Return raw and contract-normalized canonical source identities.

    Multi-room generation persists the exact model emission, while its frozen
    validator returns a semantically identical normalized plan for assembly.
    Normalization trims textual fields and repairs only the redundant
    ``metadata.requested_count`` copy to the authoritative sibling ``count``.
    Most emissions are already normalized, but both identities must be
    accepted so trailing-space normalization cannot look like source drift.
    The byte-level artifact hash remains independently mandatory above.
    """

    raw = deepcopy(dict(source_plan))
    normalized = _strip_source_plan_text(raw)
...
def _strip_source_plan_text(value: Any) -> Any:
    if isinstance(value, str):
        return value.strip()
    if isinstance(value, list):
        return [_strip_source_plan_text(item) for item in value]
    if isinstance(value, dict):
        return {
            key: _strip_source_plan_text(item)
