...
from benchmark.legend.data.scene_adapters import layout_to_scene, normalize_scene, scene_adapter_summary, scene_to_case, scene_to_layout
from benchmark.legend.evidence_config import resolve_runtime_evidence_config
from benchmark.legend.models.base_model import ModelResponseError
from benchmark.utils.io import write_json
from benchmark.legend.visualization.view_renderer import SimpleBBoxRenderer
from benchmark.legend.workflow.grouping import build_object_grouping_report
from benchmark.legend.workflow.judge_summaries import build_layout_summary, build_scene_summary, text_budget_config
...
from benchmark.legend.workflow.scoring import (
    ValidityGateResult,
    build_case_metrics,
...
from benchmark.legend.workflow.vlm_judge import (
...
def evaluate_scene(
    ...
    **compat: Any,
    ...
    consumes a legacy geometry-proxy layout internally. The scene is the
    canonical public input; layout conversion is legend compatibility only.
    """
    ...
    case_metrics.update(
        {
            ...
            "geometry_asset_count": adapter_summary["geometry_asset_count"],
            "non_geometric_asset_count": adapter_summary["non_geometric_asset_count"],
            ...
            "local_asset_ref_count": adapter_summary["local_asset_ref_count"],
            "local_asset_available_rate": adapter_summary["local_asset_available_rate"],
            "local_scene_ref_available": adapter_summary["local_scene_ref_available"],
            "local_scene_id": adapter_summary["local_scene_id"],
            "local_scene_json_path": adapter_summary["local_scene_json_path"],
            "geometry_available_rate": report.get("geometry_available_rate"),
        }
    )
...
def evaluate_layout_vlm_as_judge_v1(
    *,
    ...
    geometry_missing_flags = _geometry_missing_asset_flags(eval_layout)
    render_skipped_objects = [*layout_render_skipped_objects, *geometry_missing_flags]
    if generation_error:
    ...
    geometry_available_rate = _scene_geometry_available_rate(evaluation_scene)
    case_metrics.update(
        {
            "vlm_judge_input_mode": resolved_judge_input_mode,
            "render_evidence_used": bool(render_evidence_used),
            "json_scene_used": True,
            "geometry_available_rate": geometry_available_rate,
        }
    )
    ...
    report = {
        ...
        "debug_evidence": {
            ...
            "render_skipped_objects": render_skipped_objects,
            "geometry_missing_assets": geometry_missing_flags,
            "legend_compat": {"bbox_missing_assets": geometry_missing_flags},
...
def _geometry_missing_asset_flags(layout: dict) -> list[dict]:
    if not isinstance(layout, dict):
        return []
    missing_assets = layout.get("_non_geometric_assets")
    if not isinstance(missing_assets, list):
        missing_assets = layout.get("_non_bbox_assets")
    if not isinstance(missing_assets, list):
        return []
    flags = []
    for item in missing_assets:
        ...
        flag = _flag(
            "geometry_missing_asset",
            f"{asset_id or object_id or 'asset'} has no placement/dimensions and was skipped by geometry-only checks.",
            objects=objects,
            severity="medium",
        )
        flag["legend_type"] = "bbox_missing_asset"
        flag["asset_id"] = asset_id
        flag["object_id"] = object_id
        flag["category"] = item.get("category")
        flag["reason"] = item.get("reason") or "asset has no complete placement/dimensions geometry"
        flags.append(flag)
    return flags


def _scene_geometry_available_rate(scene: dict) -> float | None:
    ...
    geometry_count = 0
    for asset in assets:
        if _asset_has_geometry(asset):
            geometry_count += 1
    return float(geometry_count) / float(len(assets))
