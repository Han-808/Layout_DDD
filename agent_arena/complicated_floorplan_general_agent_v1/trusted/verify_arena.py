...
LOCK_PATH = ARENA_ROOT / "arena.lock.v5.json"
PREDECESSOR_LOCK = ARENA_ROOT / "arena.lock.v4.json"
EXCLUDED_NAMES = frozenset({"arena.lock.v5.json", ".DS_Store"})
...
def build_lock() -> dict[str, Any]:
    if not PREDECESSOR_LOCK.is_file() or PREDECESSOR_LOCK.is_symlink():
        raise ArenaLockError("predecessor arena.lock.v4.json is missing or linked")
    ...
    return {
        "schema_version": "sieve_agent_arena_lock_v5",
        "arena_id": "sieve-complicated-floorplan-general-agents-v2",
        "predecessor_lock": {
            "path": "arena.lock.v4.json",
...
def write_lock() -> dict[str, Any]:
    if LOCK_PATH.exists() or LOCK_PATH.is_symlink():
        raise ArenaLockError("arena.lock.v5.json already exists")
    value = build_lock()
...
def verify_lock() -> dict[str, Any]:
    if not LOCK_PATH.is_file() or LOCK_PATH.is_symlink():
        raise ArenaLockError("arena.lock.v5.json is missing or linked")
    expected = json.loads(LOCK_PATH.read_text(encoding="utf-8"))
    observed = build_lock()
    if expected != observed:
        raise ArenaLockError("arena content differs from arena.lock.v5.json")
    return observed
