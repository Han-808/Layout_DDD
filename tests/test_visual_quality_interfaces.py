...
def test_shim_names_delegate_to_scene_quality() -> None:
    ...
    assert VISUAL_QUALITY_INTERFACE_VERSION == "scene_quality_v8"
...
def test_shim_resolve_and_evaluate_work() -> None:
    ...
    assert report["interface_version"] == "scene_quality_v8"
    assert report["status"] == "failed"
    assert report["terminal_state"] == "evaluated_degraded"
