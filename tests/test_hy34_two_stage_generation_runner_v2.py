...
def test_two_stage_case_is_exactly_one_plan_one_retrieval_one_placement(tmp_path: Path) -> None:
    ...
    assert retriever.last_request["retrieval_policy"]["size_constraint_used"] is False
    assert retriever.last_request["requests"][0]["size_constraint"] is None
    assert retriever.last_request["requests"][0]["estimated_size"] == [0.5, 0.5, 0.9]
    ...
    stage_c_request = json.loads(handler.requests[1])
    stage_c_input = json.loads(stage_c_request["messages"][1]["content"])
    ...
    assert stage_c_input["schema_version"] == "hy34_generation_input_v2"
    asset = stage_c_input["asset_selection"]["objects"][0]["selected_asset"]
    assert asset["jid"] == "asset_chair"
    assert asset["size"] == [0.5, 0.5, 0.9]
    assert asset["metadata"]["default_directed_functional_side"] == "local_neg_y"
...
def test_schema_invalid_object_plan_is_not_retried(tmp_path: Path) -> None:
    invalid_plan = json.dumps({"schema_version": "hy34_object_plan_v2", "objects": []})
    retriever = _Retriever()
...
def test_contract_rejects_count_outside_brief() -> None:
    ...
    plan["objects"][0]["metadata"]["requested_count"] = 2

    with pytest.raises(ContractError, match="outside"):
        validate_object_plan(plan, brief=_brief())


def test_semantic_retrieval_preserves_planned_size_without_using_it_for_rank() -> None:
    plan = validate_object_plan(_plan(), brief=_brief())
    request = build_retrieval_request(plan)
    assert request["retrieval_policy"]["size_constraint_used"] is False
